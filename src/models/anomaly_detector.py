...
class AttentionPatchAD:
    ...
    def fit(self, dataloader):
        ...
        sigma = torch.cov(feats.T) + 1e-5*torch.eye(feats.shape[1], device=feats.device)
        self.inv = torch.linalg.inv(sigma)
    ...
    def score(self, img: torch.Tensor) -> float:
        ...
        delta = sel_f - self.mu                     # broadcast
        m_dist = (delta @ self.inv * delta).sum(-1).sqrt()   # [k]
