...
def main():
    ...
    # save models
    # ── After detector.fit(train_loader) ──
    save_dir = "saved_models"
    os.makedirs(save_dir, exist_ok=True)

    # Save model weights
    model_path = os.path.join(save_dir, "self_supervised_ad.pth")
    torch.save(detector.state_dict(), model_path)
    print(f"SelfSupervisedAttentionPatchAD weights saved to {model_path}")

    # Save feature bank for faster reload
    bank_path = os.path.join(save_dir, "feature_bank.pt")
    torch.save(detector.feature_bank, bank_path)
    print(f"Feature bank saved to {bank_path}")
...
from evaluation.test_evaluator import run_evaluation_with_existing_model
import argparse
